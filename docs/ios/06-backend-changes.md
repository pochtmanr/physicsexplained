...
3. **Google**: existing web OAuth client stays; add an iOS-type OAuth client in Google Cloud Console with the bundle id; add its client id to the Supabase Google provider's authorized client IDs (native flow uses `signInWithIdToken`).
