...
<TopicPageLayout aside={[
  ...
  { type: "term", label: "Perpendicular Axis Theorem", href: "/dictionary/perpendicular-axis-theorem" },
...
<SceneCard caption="FIG.10b — I_CM versus I about an offset axis" className="w-full">
...
There is a sibling theorem — the **<Term slug="perpendicular-axis-theorem">perpendicular-axis theorem</Term>** — that applies specifically to flat laminar bodies: for a flat plate lying in the x-y plane, I_z = I_x + I_y. The moments of inertia about the two in-plane axes sum to the moment of inertia about the axis perpendicular to the plate. For a circular disk of radius R this gives I_axis = 2·I_diameter, and so I_diameter = ¼·m·R² (compared with I_axis = ½·m·R²). Two theorems, one page of algebra, and the short list of fundamental integrals turns into a much larger table of results.
</Callout>
...
<SceneCard caption="FIG.10c — distributed bar and its equivalent point mass at k" className="w-full">
...
<SceneCard caption="FIG.10d — the inertia ellipsoid and its three principal axes" className="w-full">
  <PrincipalAxesScene />
</SceneCard>
